...
from corelibs.config import energy_shop_config, gold_shop_config, server_config, game_config
...
pr = LineProfiler() if server_config.PROFILE else None
pr_interval = int(server_config.PROFILE_INTERVAL)
...
def GetGameInfo():
    ...
    if redis_conn:
        ...
        if lastUpdate != None and currTime - float(lastUpdate) < server_config.gameRefreshInterval:
            refreshGame = False
        else:
            pipe = redis_conn.pipeline()
        ...
        retInfo['info'] = {'width':info.width, 'height':info.height, 'time':currTime, 'end_time':info.end_time, 'join_end_time':info.join_end_time, 'game_id':info.game_id, 'game_version':server_config.GAME_VERSION, 'plan_start_time':info.plan_start_time}
